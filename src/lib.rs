use std::f32::consts;

#[test]
fn it_works() {
}

 fn main() {
    // FIXME these are a function input
    let input_lat: f32 = 51.5072;
    let input_lon: f32 = 0.1275;

    let pi: f32 = consts::PI;
    //Convert input to degrees
    let lat_1: f32 = input_lat * pi / 180.;
    let lon_1: f32 = input_lon * pi / 180.;
    // The GSR80 semi-major and semi-minor axes used for WGS84 (m)
    let a_1: f32 = 6378137.;
    let b_1: f32 = 6356752.3141;
    // The eccentricity (squared) of the GRS80 ellipsoid
    let e2_1: f32 = 1. - (b_1.powf(2.)) / (a_1.powf(2.));
    // Transverse radius of curvature
    let nu_1: f32 = a_1 / (1. - e2_1 * lat_1.sin().powf(2.)).sqrt();
    // Third spherical coordinate is 0, in this case
    let H: f32 = 0.;
    let x_1 = (nu_1 + H) * lat_1.cos() * lon_1.cos();
    let y_1 = (nu_1 + H) * lat_1.cos() * lon_1.sin();
    let z_1 = ((1. - e2_1) * nu_1 + H) * lat_1.sin();

    // Perform Helmert transform (to go between Airy 1830 (_1) and GRS80 (_2))
    let small: f32 = 10.;
    let cst: f32 = 20.4894;
    let s: f32 =  cst * small.powf(-6.);
    // The translations along x, y, z axes respectively
    let tx: f32 = -446.448;
    let ty: f32 = 125.157;
    let tz: f32 = -542.060;
    // The rotations along x, y, z respectively, in seconds
    let rxs: f32 = 0.1502;
    let rys: f32 = -0.2470;
    let rzs: f32 = -0.8421;
    // In radians
    let rx: f32 = rxs * pi / (180 * 3600);
    let ry: f32 = rys * pi / (180 * 3600);
    let rz: f32 = rzs * pi / (180 * 3600);

    let x_2 = tx + (1 + s) * x_1 + (-rz) * y_1 + (ry) * z_1;
    let y_2 = ty + (rz) * x_1 + (1 + s) * y_1 + (-rx) * z_1;
    let z_2 = tz + (-ry) * x_1 + (rx) * y_1 + (1 + s) * z_1;

    // The GSR80 semi-major and semi-minor axes used for WGS84 (m)
    let a = 6377563.396;
    let b = 6356256.909;
    // The eccentricity of the Airy 1830 ellipsoid
    let e2 = 1 - (b.powf(2.)) / (a.powf(2.));
    let p = (x_2.powf(2.) + y_2.powf(2.)).sqrt();
    // Initial value
    let lat = z_2.atan2((p * (1 - e2)));
    let latold = 2 * pi;
    // this is cheating, but not sure how else to initialise nu
    let mut nu: f32 = 1.;
    // Latitude is obtained by an iterative procedure
    while (lat - latold).abs() > small.pow(-16) {
        let (lat, latold) = (latold, lat);
        let nu = a / (1 - e2 * latold.sin().powf(2.)).sqrt();
        lat = (z_2 + e2 * nu * latold.sin()).atan2(p);
    };
    let lon = y_2.atan2(x_2);
    let H = p / lat.cos() - nu;
    // Scale factor on the central meridian
    let F0 = 0.9996012717;
    // Latitude of true origin (radians)
    let lat0 = 49 * pi / 180;
    // Longtitude of true origin and central meridian (radians)
    let lon0 = -2 * pi / 180;
    // Northing & easting of true origin (m)
    let (N0, E0) = (-100000, 400000);
    let n = (a - b) / (a + b);
    // Meridional radius of curvature
    let rho = a * F0 * (1 - e2) * (1 - e2 * lat.sin().powf(2.)).powf(-1.5);
    let eta2 = nu * F0 / rho - 1;

    let M1 = (1 + n + (5/4) * n.powf(2.) + (5 / 4) * n.powf(3.)) * (lat - lat0);
    let M2 = (3 * n + 3 * n.powf(2.) + (21 / 8) * n.powf(3)) * (lat - lat0).sin() * (lat + lat0).cos();
    let M3 = ((15 / 8) * n.powf(2.) + (15 / 8) * n.powf(3.)) * (2 * (lat-lat0)).sin() * (2 * (lat + lat0)).cos();
    let M4 = (35 / 24) * n.powf(3) * (3 * (lat - lat0)).sin() * (3 * (lat + lat0)).cos();
    let M = b * F0 * (M1 - M2 + M3 - M4);

    let I = M + N0;
    let II = nu * F0 * lat.sin() * lat.cos() / 2;
    let III = nu * F0 * lat.sin() * lat.cos().powf(3.) * (5 - lat.tan().powf(2.) + 9 * eta2) / 24;
    let IIIA = nu * F0 * lat.sin() * lat.cos().powf(5.) * (61 - 58 * lat.tan().powf(2.) + lat.tan().powf(4.)) / 720;
    let IV = nu * F0 * lat.cos();
    let V = nu * F0 * lat.cos().powf(3.) * (nu / rho - lat.tan().powf(2.)) / 6;
    let VI = nu * F0 * lat.cos().powf(5.) * (5 - 18 * lat.tan().powf(2.) + lat.tan().powf(4.) + 14 * eta2 - 58 * eta2 * lat.tan().powf(2)) / 120;
    let N = I + II * (lon - lon0).powf(2.) + III * (lon - lon0).powf(4.) + IIIA * (lon - lon0).powf(6.);
    let E = E0 + IV * (lon - lon0) + V * (lon - lon0).powf(3.) + VI * (lon - lon0).powf(5.);
}
